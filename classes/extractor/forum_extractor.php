...
/**
 * Class forum_extractor
 */
class forum_extractor extends generic_extractor {
    /**
    ...
    public function extract(cm_info $cm, stdClass $course, $userid) {
        ...
        if ($forum) {
            $content->intro = content_cleaner::clean_html($forum->intro);
            $content->metadata["forumtype"] = $forum->type ?? "";
            $content->metadata["scale"] = $forum->scale ?? "";
        }
