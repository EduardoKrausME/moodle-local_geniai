 ...
 * Class url_extractor
 */
class url_extractor extends generic_extractor {
    /**
    ...
    public function supports(cm_info $cm) {
        return $cm->modname === "url";
    }

    /**
     * Extract URL activity data.
     *
     * @param \cm_info $cm Course module info.
    ...
    public function extract(cm_info $cm, stdClass $course, $userid) {
        ...
        if ($url) {
            $content->intro = content_cleaner::clean_html($url->intro);
            $content->maincontent = isset($url->externalurl) ? $url->externalurl : "";
            $content->metadata["display"] = $url->display ?? "";
        }
